'use strict';
let wall=[];
const w$=id=>document.getElementById(id),wesc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),wnorm=s=>String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^\p{L}\p{N}]+/gu,' ').trim().replace(/\s+/g,' ');
...
function mergeWildRecords(...lists){const m=new Map();for(const list of lists){for(const c of list||[]){if(!c||!c.name)continue;const key=`${String(c.record_type||'').toLowerCase()}|${wnorm(c.name)}`;if(!m.has(key))m.set(key,{...c});else m.set(key,{...m.get(key),...c})}}return[...m.values()]}
